...
7. **Multi-user / remote access** — the app assumes one local user. The server binds to 127.0.0.1 by default (`SWITCHBOARD_HOST` overrides), and `server/request-guard.ts` rejects DNS-rebound Hosts and cross-site Origins so webpages in the user's browser can't reach the API — but there's no auth layer beyond the orchestration server's opt-in bearer token if it were ever deliberately exposed.
