...
// "Kraken" rather than the host user's own git identity, since it's
...
export async function createNewRepo(dir: string): Promise<void> {
  ...
  await runGit(["-C", dir, "config", "user.name", "Kraken"], dir);
  await runGit(["-C", dir, "config", "user.email", "kraken@localhost"], dir);
...
export function branchName(taskSlug: string, sessionId: string): string {
  return `kraken/${taskSlug}-${sessionId}`;
}
...
export async function removeWorktree(dir: string, worktreePath: string): Promise<void> {
  await commitPendingChanges(worktreePath, "WIP: auto-saved by Kraken before worktree removal");
...
export const SPEC_FILE_NAME = "KRAKEN_TASKS.md";
