...
// One in-memory KV per test, opened/closed by the suite-wide hooks below —
// individual tests never manage KV lifecycle themselves.
let kv: Deno.Kv;
...
async function mustRegister(boardId: string, name: string) {
  return await registerAgent(kv, boardId, { name, role: "implementer" });
}

describe("boards", () => {
  it("concurrent creates with the same slug yield exactly one winner", async () => {
  ...
  it("are fully isolated from each other", async () => {
    const boardA = await mustCreateBoard("board-a");
    const boardB = await mustCreateBoard("board-b");
    const agentA = await mustRegister(boardA.id, "agent-a");
    const agentB = await mustRegister(boardB.id, "agent-b");
...
describe("cards: creation and dependencies", () => {
  it("a card with dependencies starts in backlog and becomes ready once they're done", async () => {
    const board = await mustCreateBoard("deps");
    const agent = await mustRegister(board.id, "agent-1");
  ...
  it("rejects a dependsOn id that does not exist on the board", async () => {
    const board = await mustCreateBoard("deps-unknown");
  ...
  it("a valid existing dependsOn still succeeds and starts in backlog", async () => {
    const board = await mustCreateBoard("deps-valid");
    const upstream = await createCard(kv, board.id, { title: "Upstream", description: "x" });
    const downstream = await createCard(kv, board.id, {
      title: "Downstream",
      description: "x",
  ...
  it("one valid + one unknown dependsOn throws naming only the unknown id", async () => {
    const board = await mustCreateBoard("deps-mixed");
    const upstream = await createCard(kv, board.id, { title: "Upstream", description: "x" });
    const err = await assertRejects(
  ...
  it("dependsOn is deduped and already-done deps start the card ready (M6)", async () => {
    const board = await mustCreateBoard("deps-hygiene");
    const agent = await mustRegister(board.id, "worker-1");
...
describe("claiming", () => {
  ...
  it("20 concurrent claims against 5 ready cards yields exactly 5 winners", async () => {
    const board = await mustCreateBoard("race-test");
    for (let i = 0; i < 5; i++) {
      await createCard(kv, board.id, { title: `Card ${i}`, description: "x" });
    }

    const agents = await Promise.all(Array.from({ length: 20 }, (_, i) => mustRegister(board.id, `agent-${i}`)));
  ...
  it("refuses a card whose fileScope overlaps an in-progress card", async () => {
    const board = await mustCreateBoard("scope");
    const agent1 = await mustRegister(board.id, "agent-1");
    const agent2 = await mustRegister(board.id, "agent-2");
  ...
  it("refuses an unregistered agent (claimCard and claimNextCard)", async () => {
    const board = await mustCreateBoard("no-agent");
  ...
  // card with scopeA in_progress; whether agent2 can claim a card with
  // scopeB tells us if the scopes were judged to overlap. Unique slug per
  // case because each call sets up its own board on the shared KV.
  async function scopesConflict(slug: string, scopeA: string[], scopeB: string[]): Promise<boolean> {
    const board = await mustCreateBoard(slug);
    const a1 = await mustRegister(board.id, `${slug}-holder`);
    const a2 = await mustRegister(board.id, `${slug}-challenger`);
  ...
  it("fileScope overlap is segment-aware: no prefix false positives, normalization handled (M6)", async () => {
    // Old prefix heuristic called this a conflict ("src/api" is a string
    // prefix of "src/apifoo.ts") — segment matching must not.
    assertEquals(await scopesConflict("sc-boundary", ["src/api"], ["src/apifoo.ts"]), false);
    // "./"-prefixed patterns are the same paths after normalization.
    assertEquals(await scopesConflict("sc-dot", ["./src/api/x.ts"], ["src/api/**"]), true);
    // "**" spans zero segments: a dir glob overlaps the bare dir path.
    assertEquals(await scopesConflict("sc-zero", ["src/api/**"], ["src/api"]), true);
    // A leading "**" can land inside any directory.
    assertEquals(await scopesConflict("sc-lead", ["**/*.test.ts"], ["src/**"]), true);
    // Genuinely disjoint trees stay parallel.
    assertEquals(await scopesConflict("sc-disjoint", ["docs/*.md"], ["src/**"]), false);
    assertEquals(await scopesConflict("sc-siblings", ["server/orchestration/routes.ts"], ["server/orchestration/service.test.ts"]), false);
  });
});

describe("agents and liveness", () => {
  it("registerAgent upserts the same name idempotently instead of creating duplicates", async () => {
    const board = await mustCreateBoard("idempotent");
  ...
  it("reaper releases an expired lease back to ready", async () => {
    const board = await mustCreateBoard("reaper-lease", { leaseMs: 1 });
    const agent = await mustRegister(board.id, "worker-1");
    const card = await createCard(kv, board.id, { title: "Task", description: "x" });
  ...
  it("reaper does not touch blocked cards even past lease", async () => {
    const board = await mustCreateBoard("reaper-blocked", { leaseMs: 1 });
    const agent = await mustRegister(board.id, "worker-1");
    const card = await createCard(kv, board.id, { title: "Task", description: "x" });
    const claimed = await claimCard(kv, board.id, agent.id, card.id);
    assert(claimed.ok);
  ...
  it("reaper marks an agent past the heartbeat threshold offline", async () => {
    const board = await mustCreateBoard("reaper-heartbeat", { heartbeatMs: 1 });
    const agent = await mustRegister(board.id, "worker-1");
  ...
  it("heartbeat renews the lease on the agent's current card", async () => {
    const board = await mustCreateBoard("heartbeat-renew", { leaseMs: 50 });
    const agent = await mustRegister(board.id, "worker-1");
    const card = await createCard(kv, board.id, { title: "Task", description: "x" });
    const claimed = await claimCard(kv, board.id, agent.id, card.id);
    assert(claimed.ok);
  ...
  it("heartbeat revives a reaped-offline agent to idle unless told otherwise (M6)", async () => {
    const board = await mustCreateBoard("revive", { heartbeatMs: 1 });
    const agent = await mustRegister(board.id, "sleepy");
  ...
  it("heartbeat reports unread-message and new-event counts without consuming them", async () => {
    const board = await mustCreateBoard("heartbeat-counts");
    const agent = await mustRegister(board.id, "worker-1");
    const other = await mustRegister(board.id, "worker-2");
...
describe("messaging", () => {
  ...
  it("two agents exchange a direct message about a shared interface", async () => {
    const board = await mustCreateBoard("msg-direct");
    const backend = await mustRegister(board.id, "backend-worker");
    const frontend = await mustRegister(board.id, "frontend-worker");
  ...
  it("broadcast is a shared ring each agent reads once via its own cursor", async () => {
    const board = await mustCreateBoard("msg-broadcast");
    const lead = await mustRegister(board.id, "lead");
    const workerA = await mustRegister(board.id, "worker-a");
    const workerB = await mustRegister(board.id, "worker-b");
  ...
  it("sendMessage rejects unregistered from/to agents", async () => {
    const board = await mustCreateBoard("msg-unregistered");
    const agent = await mustRegister(board.id, "worker-1");
...
describe("events", () => {
  it("watchEvents advances the cursor and only returns events past it", async () => {
    const board = await mustCreateBoard("watch-events");
    const agent = await mustRegister(board.id, "watcher");
  ...
  it("sweepBoard compacts events older than the board's retention window (M6)", async () => {
    const board = await mustCreateBoard("compaction", { eventRetentionMs: 1 });
  ...
  it("fresh agents start with cursors at now — no self-registration noise (M6)", async () => {
    const board = await mustCreateBoard("cursor-init");
    const early = await mustRegister(board.id, "early-bird");
    ...
    const late = await mustRegister(board.id, "late-joiner");
    ...
    await mustRegister(board.id, "late-joiner");
    assertEquals((await heartbeat(kv, board.id, late.id)).newEvents, 1);
  });
});
