...
Deno desktop app: Kraken (agent-session supervisor UI, `src/` + `server/kraken/`) plus a headless orchestration server (`server/orchestration/`) — a multi-board Kanban store with an MCP surface for coordinating multiple Claude Code sessions. Architecture: `README.md`. Orchestration design + build order: `agent-kanban-orchestration-plan-v2-1.md` (all milestones M1–M6 implemented).
